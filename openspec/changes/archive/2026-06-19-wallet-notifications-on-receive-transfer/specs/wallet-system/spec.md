...
O sistema SHALL possibilitar o envio de moedas entre membros da comunidade de forma segura e manter rastreabilidade com as atividades de governança. Toda lógica de transferência MUST ser processada exclusivamente no servidor (Edge Functions) para garantir a integridade. Adicionalmente, o sistema SHALL emitir uma notificação para o destinatário informando sobre o crédito bem-sucedido.
...
#### Scenario: Notificação ao Destinatário
- **WHEN** uma transferência é concluída com sucesso no ledger.
- **THEN** a Edge Function SHALL criar uma notificação no Notifications Hub endereçada ao usuário destinatário contendo o nome ou email do remetente e o valor transferido.
