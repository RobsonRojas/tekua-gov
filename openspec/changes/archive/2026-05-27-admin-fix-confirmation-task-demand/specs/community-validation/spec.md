...
Membros da Vila **SHALL** confirmar a realização de trabalhos publicados no mural para garantir a transparência e consenso. Toda nova tarefa **MUST** herdar o número mínimo de validações (threshold) configurado globalmente no momento de sua criação.
...
#### Scenario: Herança de threshold global
- **WHEN** Um novo trabalho ou tarefa é submetido.
- **THEN** O backend (`api-work`) consulta as configurações de governança e atribui o `validation_threshold` atual como o requisito `min_confirmations` para a tarefa criada.
